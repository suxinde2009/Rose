...
void mkwin_controller::theme_into_widget(unit* u)
{
	...
	widget->set_tooltip(dgettext("wesnoth-lib", "Return"));
...
void mkwin_controller::theme_goto_top(bool auto_select)
{
	...
	widget->set_tooltip(dgettext("wesnoth-lib", "Go into"));
...
*/
	...
	if (!mistaken_ids.empty()) {
		...
		const std::string err = vgettext("wesnoth-lib", "widget($widget) has rect, must set id!", symbols);
...
void mkwin_controller::erase_widget()
{
	...
	if (has_child) {
		...
		if (!all_spacer) {
			...
			ss << vgettext("wesnoth-lib", "$id holds grid that not all spacer. Do you want to delete?", symbols);
		...
		if (is_top_unit) {
			if (!id.empty()) {
				if (callback_sstr_.find(id) == callback_sstr_.end()) {
					...
					err = vgettext("wesnoth-lib", "On theme top, duplicated '$id' value!", symbols);
				}
			}
			if (!u->fix_rect()) {
				err = vgettext("wesnoth-lib", "widget($widget) is on theme top, must set rect!", symbols);
			}
		}
...
/*
		if (u->fix_rect() && u->cell().rect_cfg["ref"].empty()) {
			err = vgettext("wesnoth-lib", "widget($widget) has rect, must set ref!", symbols);
		}
*/
		if (!err.empty()) {
			if (show_error) {
				if (is_top_unit) {
					...
					if (in_theme_top()) {
						...
						err = vgettext("wesnoth-lib", "Widget on second layer has error. Go into $widget to get error detail.", symbols);
	...
	if (u->is_stacked_widget()) {
		...
		for (std::vector<unit::tchild>::const_iterator it = children.begin(); it != children.end(); ++ it) {
			...
			if (child.is_all_spacer()) {
				err = vgettext("wesnoth-lib", "widget($widget) has all spacer layer!", symbols);
				if (show_error) {
					gui2::show_message(gui_->video(), "", err);
				}
				return false;
...
void mkwin_controller::quit_confirm(EXIT_STATUS mode, bool dirty)
{
	...
	if (dirty) {
		...
		std::string str = vgettext("wesnoth-lib", "Changes in the $window since the last save will be lost.", symbols);
...
void mkwin_controller::unpack_widget_tpl(unit* u)
{
	...
	ss << vgettext("wesnoth-lib", "Once unpake, can not be back to template. Do you want to unpake $id?", symbols);
	int res = gui2::show_message(gui_->video(), "", ss.str(), gui2::tmessage::yes_no_buttons);
	if (res != gui2::twindow::OK) {
		return;
	}
