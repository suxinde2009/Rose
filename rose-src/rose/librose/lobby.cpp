...
void tlobby::tchat_sock::handle_command(const char* param[])
{
	...
	if (state_ == s_consult) {
		if (!strcasecmp(param[0], "NICKERR")) {
			...
			symbols["account"] = tintegrate::generate_format(dsgettext("wesnoth-lib", "Register, config account"), "yellow");
			ss << "$nick is invalid nick when chat! To set up: Press $account in \"Player profile\" Setting, and set valid nick when chat.";
			VALIDATE(utils::to_int(param[2]) != 1, vgettext("wesnoth-lib", ss.str().c_str(), symbols));
