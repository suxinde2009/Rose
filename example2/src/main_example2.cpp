...
static int do_gameloop(int argc, char** argv)
{
	...
	try {
		...
		res = true;
		do {
			int retval = preferences::show_preferences_dialog(game.disp(), res);
			if (retval == gui2::texample2::CHANGE_LANGUAGE) {
				if (game.change_language()) {
					t_string::reset_translations();
					image::flush_cache();
				}
				
			} else if (!preferences::is_resolution_retval(retval)) {
				break;
			}
			res = false;
		} while (true);
