...
void init_layout_mode()
{
	...
	if (horizontal_anchor.empty()) {
		...
		horizontal_anchor.insert(std::make_pair(theme::FIXED, tanchor(theme::FIXED, vgettext("wesnoth-lib", "$field1 fixed, $field2 fixed", symbols), true)));
		horizontal_anchor.insert(std::make_pair(theme::TOP_ANCHORED, tanchor(theme::TOP_ANCHORED, vgettext("wesnoth-lib", "$field1 fixed, $field2 scalable", symbols), true)));
		horizontal_anchor.insert(std::make_pair(theme::BOTTOM_ANCHORED, tanchor(theme::BOTTOM_ANCHORED, vgettext("wesnoth-lib", "$field1 scalable, $field2 fixed", symbols), true)));
	}
...
void tcontrol_setting::pre_rect(twindow& window)
{
	...
	vstr.push_back(vgettext("wesnoth-lib", "ref's $field", symbols));
	...
	vstr.push_back(vgettext("wesnoth-lib", "ref's $field", symbols));
	symbols["field"] = tintegrate::generate_format("x1", "red");
	vstr.push_back(vgettext("wesnoth-lib", "My $field", symbols));
...
std::string tcontrol_setting::calculate_rect_coor(twindow& window, int n) const
{
	...
	if (isinteger(val.c_str())) {
		...
		if (!op) {
			if (ch == '+' || ch == '-') {
				err << vgettext("wesnoth-lib", "$id is absolute type, must not be contain '+' or '-'!", symbols);
			}
		} else if (op == 1) {
			val = std::string("=") + val;
		...
		err << vgettext("wesnoth-lib", "$id must be integer!", symbols);
	}
	if (!err.str().empty()) {
