...
#define BLIT_TRANSL_888(src, dst)               \
    do {                            \
        Uint32 s = src;                     \
    Uint32 d = dst;                     \
	Uint8 src_a = (s & 0xFF000000) >> 24;		\
    Uint8 src_r = (s & 0x00FF0000) >> 16;		\
	Uint8 src_g = (s & 0x0000FF00) >> 8;	\
	Uint8 src_b = s & 0x000000FF;	\
	Uint8 dst_a = (d & 0xFF000000) >> 24;		\
	Uint8 dst_r = (d & 0x00FF0000) >> 16;	\
	Uint8 dst_g = (d & 0x0000FF00) >> 8;	\
	Uint8 dst_b = d & 0x000000FF;		\
	if (!dst_a) {							\
		dst = s;							\
		break;								\
	}										\
	if (dst_a != 255) {					\
		dst_a = (((255 - dst_a) * src_a) >> 8 ) + dst_a;	\
	}						\
