...
public:
	...
	{
	public:
		sort_func(void* caller, bool (*callback)(void*, tgrid&, tgrid&)) : caller_(caller), callback_(callback)
		{}

		bool operator()(titem* a, titem* b) const
		{
			return callback_(caller_, a->grid, b->grid);
		}

	private:
		void* caller_;
		bool (*callback_)(void*, tgrid&, tgrid&);
	};
	...
	void sort(void* caller, bool (*callback)(void*, tgrid&, tgrid&)) {
		std::stable_sort(items_.begin(), items_.end(), sort_func(caller, callback));
	}
